...
<div class="grid-margin stretch-card">
    <div class="card">
        ...
        <div class="card-body">
            <a class='btn btn-info  btn-gradient-info' href="{{ route('guru.create') }}"><i
            ...
            columnDefs: [{
                ...
                render: function (data, type, row) {
                    ...
                    <a href=":edit" class='btn btn-warning  btn-sm'><i
                        ...
                        <a href="#" class='btn btn-danger  btn-sm delete' data-id="${row.nip}"><i
                                class="mdi mdi-delete"></i> </a>
                    `.replace(':edit', `{{ route('guru.edit', 'nip') }}`.replace('nip', row.nip));
                }
            }]
        });
    });
...
<script>
    $(document).on('click', '.delete', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('guru.destroy', ':nip') }}".replace(':nip', nip),
                    type: "POST",
                    data: {
                        '_token': "{{ csrf_token() }}",
                        '_method': 'POST'
